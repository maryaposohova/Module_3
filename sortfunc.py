...
def insertion_soft(ls):    #
    for i in range(1, len(ls)):  # цикл проходит по всем элементам со второго, первый элемент считаем отсортированным
        ...
        while ls[j] > key and j >= 0: # сдвигая элементы вправо, ищем правильную позицию для кей(нашего екущего элемента).
            ls[j+1] = ls[j]           # Первое условие проверяет, нужно ли сдвигать, а второе, что мы не выйдем за пределы слева
            j-=1
